...
function themeRequiring(array $requires): FakeThemeSource
{
...
it('reads what the theme requires', function () {
...
it('runs a check the theme requires', function () {
    ...
    $tester = cli(['path' => $project], themeRequiring(['fresh-project']));
...
it('stops when a required check fails', function () {
    ...
    $tester = cli(['path' => laravelProject()], themeRequiring(['fresh-project']));

    expect($tester->getStatusCode())->toBe(Command::FAILURE)
        ->and($tester->getDisplay())->toContain('fresh-project');
});

it('does not run a check the theme did not require', function () {
    $tester = cli(['path' => laravelProject()], themeRequiring([]));
...
it('continues past a required check with --force', function () {
    $tester = cli(['path' => laravelProject(), '--force' => true], themeRequiring(['fresh-project']));
...
it('refuses a check the driver does not offer', function () {
    $tester = cli(['path' => laravelProject()], themeRequiring(['sunshine']));
